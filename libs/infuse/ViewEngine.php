...
class ViewEngine
{
	 ...
	 */
	public function compileLess($input, $cacheFile, $output)
	{
		try {
			// load the cache
	 ...
	 * @return boolean result
	 */
	public function compileJs($dir, $cacheFile, $output, $productionLevel = false)
	{
		try {
	 ...
	 * @return boolean result
	 */
	public function compileAngularTemplates($dir, $cacheFile, $output)
	{
		try {
			// NOTE js files get appended in order by filename
            // to change the order of js files, change the filename
	 ...
	 */
	public function computeVersionNumbers(array $dirs, array $cacheExts, $output, $base = '', $stripPrefix = '')
	{
		try {
			$versionNums = array();
