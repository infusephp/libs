...
class Queue
{
	 ...
	 */
    public static function install($echoOutput = false)
    {
        $type = self::type();
	 ...
	 * @return boolean success
	 */
    public static function enqueue( $queue, $message, $parameters = array() )
    {
        $type = self::type();
        ...
        } elseif ($type == QUEUE_TYPE_SYNCHRONOUS) {
	 ...
	 */
    public static function dequeue($queue, $n = 1)
    {
        $type = self::type();
	 ...
	 */
    public static function receiveMessage($queue, $message)
    {
        $success = true;
