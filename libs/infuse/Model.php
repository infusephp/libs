...
abstract class Model extends Acl
{
	 ...
	 * @return boolean
	 */
    public static function isIdProperty($propertyName)
    {
	 ...
	 * @return int total
	 */
    public static function totalRecords( array $where = array() )
    {
	 ...
	 */
    public static function suggestSchema()
    {
        if( !static::hasSchema() )
	 ...
	 */
    public static function updateSchema($cleanup = false)
    {
        if( !static::hasSchema() )
                ...
                if ( isset( $property[ 'validate' ] ) ) {
                    if ( is_callable( $property[ 'validate' ] ) ) {
                        if( !call_user_func_array( $property[ 'validate' ], array( &$value ) ) )
                            $thisIsValid = false;
                    } elseif( !Validate::is( $value, $property[ 'validate' ] ) )
                        $thisIsValid = false;
